...
BASE='f1a24af692a7e63722394657ef6728616b655716'
...
for name in html:
 old=git('show',BASE+':'+name);new=(ROOT/name).read_text()
 if normalized(old)!=normalized(new):errors.append({'page':name,'error':'Non-presentation HTML changed'})
 if not re.search(r'http-equiv=[\"\']refresh',old,re.I) and (new.count('data-dr-theme-init')!=1 or new.count('data-theme-toggle hidden')!=1):errors.append({'page':name,'error':'Missing or duplicate theme controls'})
...
report={'baseline':BASE,'html_pages':len(html),'redirect_pages':len(redirects),'application_shells':len([x for x in html if x.startswith('drvcam/')]),'preservation_errors':errors,'missing_local_targets':missing,'missing_fragments':anchors,'routes':[{'file':n,'redirect':n in redirects} for n in html]}
